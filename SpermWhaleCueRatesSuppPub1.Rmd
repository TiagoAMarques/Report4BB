...
# select only those
index.min<-which(table(tags$location,tags$year)>nmin)
# create an object to hold all possible location-year's independent of the nomber of tags (1 is enough to be in)
...
## Estimating the precision of the cue rate for a new year-location combinatioon

So we estimated the mean cue rate at a new site-year to be `r round(exp(summary(crglmer)$coefficients[1,1]),2)`. To estimate the precision on this mean estimate for cue rate we would need to propagate the variability associated with the random effects into the point estimate of the intercept of the model. 

How can that be done? About this same question, Bolker et al. note here under their GLMM FAQ section on [Predictions and/or confidence (or prediction) intervals on predictions](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#predictions-andor-confidence-or-prediction-intervals-on-predictions) that "none of the following approaches takes the uncertainty of the random effects parameters into account..." and suggest that "if you want to take RE parameter uncertainty into account, a Bayesian approach is probably the easiest way to do it.". 
...
This ignores variability in estimated random effects, but to get a 95% confidence interval (CI) we can simply use the percentile method, leading to a mean estimate of `r round(exp(summary(crglmer)$coefficients[1,1]),2)` and 95% confidence intervals of `r round(quantile(est.mean.crs,0.025),2)`-`r round(quantile(est.mean.crs,0.975),2)`.
...
I implement here below this second bootstrap, which would account for the variability in estimating the random effects, by sampling year-location combinations. In hindsight, I might not have done just quite what BB was proposing, so I am coining this as option 3a.  This might actually by a weird hybrid without any theoretical justification.
...
As before, we can obtain a 95% CI using the percentile method: `r round(quantile(res.boot,0.025),2)`-`r round(quantile(res.boot,0.975),2)`. As expected, this 95% CI is, even if just ever so slightly, wider than when ignoring the component of variation due to the random effects estimation. We can see that overlaid with the point and interval estimates presented before:
...
1. simulate data from the fitted model, that is, for each original data point (i.e. each tag)
    ...
    - draw a Gamma observation with a mean given by the `int+rey+rel` (but which variance? [see comment above](#ilink1))
...
Actually, we can easily simulate new data from this model via function `simulate`. Unfortunately, the simulated data looks nothing like the real data, having considerable higher variability than the original data <a id="ilink2"></a> 
...
I actual tried all the options explicitly, `re.form=NULL`, `re.form=NA`, `re.form=0` and `re.form=~(1|location)+(1|fyear)`, but could not manage to get simulated data that looks like the observed data from any of these options. Unrelated but still quite strange, the code with with anything but the default option of not explicitly specifying the argument `re.form` does not run in the .Rmd, even though it does run in the command line.
...
##### Implementing option 3b

The results below make no sense, and this might be due to the way `simulate` is working, in particular given that if we `simulate` from a model with year and location as random effects, the data looks consistent with the real data, but if we `simulate` from the random effects model, it does not.
...
legend("topright",legend=c("Estimated cue rate","95% CI ignoring RE var","95% CI including RE var 3a","95% CI ignoring RE var","95% CI including RE var 3b"),lty=2,col=c(1,3,4,5),inset=0.05)
```
