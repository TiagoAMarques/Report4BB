...
In this paper we report on estimates of cue rates for sperm whales at a selection of time and year combinations that were sampled, but the main interest is in estimating a mean cue rate, and corresponding precision, for a time and location that might not have been observed.
...
Note that the above does not sample the right distributions, because it ignores the potential correlation between the variance of the random effects. However, when I looked at said correlation via the Bayesian implementation, option 4 [below](#ilinkR), there was no evidence for it to be present, so this might not be too bad as an approximation.
...
This section is Work in Progress and might become in fact deprecated. 

All the problems might have a common source and be down to an issue with either the `glmer` fit or to the way `simulate` works over a Gamma GLMM.
...
Note that in this procedure there are a considerable number of warnings regarding the `glmer` fit which render this approach somewhat unreliable.
...
Here I attempt to implement the GLMM model within a Bayesian context. I chose NIMBLE for implementation. Because we run into several issues with the results from the Bayesian implementation not adding up to the results from `lme4::glmer`, we compare the results of:

1. The `lme4::glmer`
2. Nimble (MCMC)
3. `glmmTMB`
4. Nimble with Lapalce apporximation (MLE's)
...
Implement the Nimble-Laplace apporximation
...
Implement the glmmTMB approach
...
However, the results are somewhat inconsistent with what we got in `glmer`. 

So for the intercept
...
while for the dispersion parameter
...
and the random effects variances
...
When we compare the estimated values under `glmer`, `glmmTMB`, NIMBLE MLE's via Lapace apporximations and the posteriors from NIMBLE, things do not add up
...
In summary, the `lme4::glmer` was off for both the intercept and the dispersion parameter, consistent across the other 3 implementations, while for the random effect variances the Nimble MLE's (via Lapace approximation) and `glmmTMB` were virtually identical, with Nimble MCMC and `lme4::glmer` showing inconsistent results.

<a id="ilinkR"></a> Just out of curiosity, I took a peak at what was the correlation between the random effects standard deviations, and it looks like there is none, even if one tests it formally (P-value=`r round(cor.test(test$samples[,134],test$samples[,135])$p.value,3)`).
...
Here we present the comparison of option 2, 3a and 4. 3b is not shown given the issues mentioned above. I think I prefer the BAyesian option, since thta seems to be the only one that avoids approximations which might be the cause for discrepancies.
